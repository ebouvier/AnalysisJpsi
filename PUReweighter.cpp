...
PUReweighter::PUReweighter(bool isSemiMu,std::string _dataFilename,  PUProfile profile/* = PUProfile::S10*/, Systematic syst/* = Systematic::NOMINAL*/):
  puHisto(NULL) {
    const std::string path = "../My_PU_Histos/";
    const std::string dataFileName = path + _dataFilename;
   ...
   for (int i = 1; i <= dataHisto->GetNbinsX(); i++) {
     ...
     double coef = index < (int) profile_coefs.size() ? profile_coefs[index] : 0.;
    ...
    // // Create MC PU histogram
    // TH1D* mcHisto = new TH1D("pileup_mc", "pileup", 120, 0, 60);
    // mcHisto->SetDirectory(NULL);
    // for (int i = 1; i <= mcHisto->GetNbinsX(); i++) {
    //   double coef = (i - 1) < (int) profile_coefs.size() ? profile_coefs[i - 1] : 0.;
...
double PUReweighter::weight(float interactions) const {
  ...
  if (puHisto->GetBinContent(bin)>2000) return 0;
